...
echo "Production Folder:"$uasProdFolder
#
...
echo "Flight Path:"$flightPath
flightFolder=`find -maxdepth 1 -name "uas_2*" -type d`
echo "Flight Folder:"$flightFolder
...
latMIN=$(mysql wheatgenetics -h apate -u mlucas  -se "SELECT MIN(uas_latitude) FROM uas_images where flight_id LIKE '$flightID'")
latMAX=$(mysql wheatgenetics -h apate -u mlucas  -se "SELECT MAX(uas_latitude) FROM uas_images where flight_id LIKE '$flightID'")
longMIN=$(mysql wheatgenetics -h apate -u mlucas  -se "SELECT MIN(uas_longitude) FROM uas_images where flight_id LIKE '$flightID'")
longMAX=$(mysql wheatgenetics -h apate -u mlucas  -se "SELECT MAX(uas_longitude) FROM uas_images where flight_id LIKE '$flightID'")
echo $latMIN
echo $latMAX
echo $longMIN
echo $longMAX
...
#
UAS_XMIN=$(mysql wheatgenetics -h apate -u mlucas  -se "SELECT MIN(uas_position_x) FROM uas_images where flight_id LIKE '$flightID'")
UAS_XMAX=$(mysql wheatgenetics -h apate -u mlucas  -se "SELECT MAX(uas_position_x) FROM uas_images where flight_id LIKE '$flightID'")
UAS_YMIN=$(mysql wheatgenetics -h apate -u mlucas  -se "SELECT MIN(uas_position_y) FROM uas_images where flight_id LIKE '$flightID'")
UAS_YMAX=$(mysql wheatgenetics -h apate -u mlucas  -se "SELECT MAX(uas_position_y) FROM uas_images where flight_id LIKE '$flightID'")
echo $UAS_XMIN
echo $UAS_XMAX
echo $UAS_YMIN
echo $UAS_YMAX
